...
TCPRdtSender::TCPRdtSender():expectSequenceNumberSend(0), window(), lastAckAmount(0)
{
}
			...
			if (flag == -1) {
                ...
                if (lastAckAmount >= 1) {
                    if (window.size() > 0) {
                        ...
                        pUtils->printPacket("发送方发送报文", window[0]);
					    pns->startTimer(SENDER, Configuration::TIME_OUT, window[0].seqnum);			//启动发送方定时器
					    pns->sendToNetworkLayer(RECEIVER, window[0]);								//调用模拟网络环境的sendToNetworkLayer，通过网络层发送到对方
                    }
                }
			}
            lastAckAmount = 0; // 正确收到确认包时重置次数
...
void TCPRdtSender::timeoutHandler(int seqNum) {
    ...
    for (int i = 0; i < window.size() && i < 4; i++) {
        if (window[i].seqnum == seqNum) {
            flag = true;
            pns->stopTimer(SENDER, window[i].seqnum);
            pns->startTimer(SENDER, Configuration::TIME_OUT, window[i].seqnum);
            break;
        }
    }
    if (flag == true) {
        pUtils->printPacket("发送方定时器时间到，重发窗口的起始报文", window[0]);
	    pns->stopTimer(SENDER, window[0].seqnum);										//首先关闭定时器
	    pns->startTimer(SENDER, Configuration::TIME_OUT, window[0].seqnum);			//重新启动发送方定时器
	    pns->sendToNetworkLayer(RECEIVER, window[0]);			//重新发送数据包
    }
}